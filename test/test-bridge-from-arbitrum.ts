...
const ARBITRUM_SEPOLIA_VALIDATOR = process.env.ARBITRUM_VALIDATOR_ADDRESS || "0xbD57b37FEf0fda7151a0C0BdA957aE37BD84ab6B" as Address;
const ARBITRUM_SEPOLIA_OFT = process.env.ARBITRUM_OFT_ADDRESS || "0x004690Ee41C0Dd2AcEf094D01b93b60aa9a06bb9" as Address; // Arbitrum Sepolia OFT
...
const SOURCE_CHAIN_NAME = "Arbitrum Sepolia";
...
async function main() {
  ...
  console.log("📝 Validator contract:", ARBITRUM_SEPOLIA_VALIDATOR);
  console.log("💰 OFT Token:", ARBITRUM_SEPOLIA_OFT);
  ...
  const balance = await publicClient.readContract({
    address: ARBITRUM_SEPOLIA_OFT as Address,
    abi: erc20Abi,
  ...
  const permit = {
    permitted: {
      token: ARBITRUM_SEPOLIA_OFT as Address,
      amount: amount,
    },
    spender: getAddress(ARBITRUM_SEPOLIA_VALIDATOR) as Address,
    nonce: nonce,
    deadline: deadline,
  };
  ...
  const fee = await publicClient.readContract({
    address: getAddress(ARBITRUM_SEPOLIA_VALIDATOR) as Address,
    abi: validatorAbi,
    functionName: "quoteBridge",
    args: [
      ARBITRUM_SEPOLIA_OFT as Address,
      DESTINATION_EID,
